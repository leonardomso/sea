...
namespace Sea.Tests.EditMode
{
    public sealed class SeaChartMotionTests
    {
        ...
        [Test]
        public void Motion_timeline_carries_a_ship_as_far_as_the_publish_heartbeat_and_no_further()
        {
            ...
            Assert.That(timeline.Sample(13d).Position.x, Is.EqualTo(30f).Within(0.001f));
            Assert.That(timeline.Sample(13d).HeadingDegrees, Is.EqualTo(30f).Within(0.001f));

            // The server only sends a ship again once her reckoning has drifted, and on a ten
            // tick heartbeat regardless, so holding the course that long is drawing what the
            // server knows. Past the heartbeat something is wrong and she waits to be told.
            Assert.That(timeline.Sample(21d).Position.x, Is.EqualTo(110f).Within(0.001f));
            Assert.That(
                timeline.Sample(40d).Position.x,
                Is.EqualTo(110f).Within(0.001f),
                "Beyond the heartbeat a ship must hold her last reckoning, not sail off the chart.");
        }
