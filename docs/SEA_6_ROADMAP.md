# Sea — Next steps roadmap

Date: 2026-09-02
Status: proposal. On approval this file replaces `PLAN.md` as the source of
truth named in `AGENTS.md`. Until then `PLAN.md` stands and nothing here is
implemented.
...
Stop the current roadmap at Phase 17 and start Milestone 1 of this roadmap
now. Do not finish `PLAN.md` Phase 18 and 19 first.
...
What is kept from Phase 18: the scale smoke (`pnpm runtime:test:scale-isolated`)
...
- `pnpm verify`, then `pnpm verify:full` with the scale smoke and the 5,000
  client proof from `PLAN.md` Phase 18 against the new model. Keep the
...
- Update `README.md`, `CONTEXT.md`, and `AGENTS.md` pointers.
...
## 5. Decisions needed before Milestone 1 starts

Each has a recommendation. Milestone 1 assumes the recommendation.

1. Adopt this roadmap and retire `PLAN.md` Phase 18 and 19 as written.
   Recommended: yes, with the Phase 18 gates folded into sub-phase 1f.
2. Identity stays anonymous and local until Milestone 5, with `AccountId`
   added now. Recommended: yes. Better Auth needs a deployment to be worth
   testing, and nothing in Milestones 1 to 4 depends on it.
3. WASD becomes manual steering and the camera follows the ship. Chart pan
   moves to middle mouse. Recommended: yes, per Mechanics section 1.1.
4. Ship to ship collision waits for `Ram` in Milestone 3. Recommended: yes.
   The rule exists for ramming and changes nothing in a fight before then.
5. Content moves to embedded JSON validated by `Init`. Recommended: yes, per
   Technical section 4.1. It also lets balance tests read the same files the
   server ships.
6. The four design docs are committed to the repo as the design of record.
   Recommended: yes. They are untracked today.
