...
public sealed class ContentValidationTests
{
    ...
    public static TheoryData<string, GameContent> RejectedContent => new()
    {
        ...
        { "patrol: maximum speed must be positive.", Npc(npc => npc with { MaximumSpeedSquares = 0f }) },
        { "patrol: aggro range must be between 0 and 110.", Npc(npc => npc with { AggroRangeSquares = -1f }) },
        { "patrol: aggro range must be between 0 and 110.", Npc(npc => npc with { AggroRangeSquares = 12f }) },
        { "patrol: desired range must be between 0 and 110.", Npc(npc => npc with { DesiredRangeSquares = 12f }) },
